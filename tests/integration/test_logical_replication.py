...
class TestLogicalReplication(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        tap_keap_postgres.dump_catalog = lambda catalog: True
    ...
    def test_logical_replication(self):
        streams = tap_keap_postgres.do_discovery(self.config)

        awesome_stream = [
            s for s in streams if s["tap_stream_id"] == f"public-{self.table_name}"
        ][0]
        ...
        with contextlib.redirect_stdout(my_stdout):
            state = tap_keap_postgres.do_sync(
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    f"update {self.table_name} set colour='purple' where name='betty';"
                )
                cur.execute(
                    f"alter table {self.table_name} add column nice_flag bool default true;"
                )
                insert_record(
                    cur,
                    self.table_name,
