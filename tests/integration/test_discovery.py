...
import psycopg2
import tap_keap_postgres
import tap_keap_postgres.db as post_db
from psycopg2.extensions import quote_ident
from singer import metadata
from tap_keap_postgres.discovery_utils import BASE_RECURSIVE_SCHEMAS

from ..utils import ensure_test_table, get_test_connection, get_test_connection_config
...
class TestStringTableWithPK(unittest.TestCase):
    ...
    def test_catalog(self):
        ...
        with contextlib.redirect_stdout(my_stdout):
            streams = tap_keap_postgres.do_discovery(conn_config)
    ...
    def test_catalog(self):
        ...
        self.assertEqual(
            metadata.to_map(stream_dict.get("metadata")),
            {
                ...
                ("properties", "size smallint"): {
                    "inclusion": "available",
                    "sql-datatype": "smallint",
                    "selected-by-default": True,
                },
                ("properties", "size bigint"): {
                    "inclusion": "available",
                    "sql-datatype": "bigint",
                    "selected-by-default": True,
                },
...
class TestDecimalPK(unittest.TestCase):
    ...
    def setUp(self):
        table_spec = {
            "columns": [
                {"name": "our_decimal", "type": "numeric", "primary_key": True},
...
class TestDatesTablePK(unittest.TestCase):
    ...
    def setUp(self):
        table_spec = {
            "columns": [
                {"name": "our_date", "type": "DATE", "primary_key": True},
...
class TestFloatTablePK(unittest.TestCase):
    ...
    def setUp(self):
        table_spec = {
            "columns": [
                {"name": "our_float", "type": "float", "primary_key": True},
...
class TestJsonTables(unittest.TestCase):
    ...
    def test_catalog(self):
        ...
        self.assertEqual(
            {
                "properties": {
                    "our_secrets": {"type": ["null", "object", "array"]},
...
class TestUUIDTables(unittest.TestCase):
    ...
    def setUp(self):
        table_spec = {
            "columns": [
                {"name": "our_pk", "type": "uuid", "primary_key": True},
    ...
    def test_escaping_values(self):
        ...
        with get_test_connection() as conn:
            with conn.cursor() as cur:
                query = tap_keap_postgres.sync_strategies.logical_replication.create_hstore_elem_query(
                    elem
                )
                self.assertEqual(
                    query.as_string(cur),
...
class TestEnumTable(unittest.TestCase):
    ...
    def setUp(self):
        table_spec = {
            "columns": [
                {"name": "our_mood_enum_pk", "type": "mood_enum", "primary_key": True},
    ...
    def test_catalog(self):
        ...
        with get_test_connection() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                cur.execute(
                    """INSERT INTO "CHICKEN TIMES" (our_mood_enum_pk, our_mood_enum) VALUES ('sad', 'happy')"""
                )
                cur.execute("""SELECT * FROM  "CHICKEN TIMES" """)
                ...
                self.assertEqual(
                    ...
                    {
                        (): {
                            "table-key-properties": ["our_mood_enum_pk"],
                            "database-name": "postgres",
                            "schema-name": "public",
                            "is-view": False,
                            "row-count": 0,
                        },
                        ("properties", "our_mood_enum_pk"): {
                            "inclusion": "automatic",
                            "sql-datatype": "mood_enum",
                            "selected-by-default": True,
                        },
                        ("properties", "our_mood_enum"): {
                            "inclusion": "available",
                            "sql-datatype": "mood_enum",
                            "selected-by-default": True,
                        },
                ...
                self.assertEqual(
                    {
                        "properties": {
                            "our_mood_enum": {"type": ["null", "string"]},
...
class TestMoney(unittest.TestCase):
    ...
    def setUp(self):
        table_spec = {
            "columns": [
                {"name": "our_money_pk", "type": "money", "primary_key": True},
    ...
    def test_catalog(self):
        ...
        with get_test_connection() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                cur.execute(
                    """INSERT INTO "CHICKEN TIMES" (our_money_pk, our_money) VALUES ('$1.24', '$777.63')"""
                )
                cur.execute("""SELECT * FROM  "CHICKEN TIMES" """)
                ...
                self.assertEqual(
                    ...
                    {
                        (): {
                            "table-key-properties": ["our_money_pk"],
                            "database-name": "postgres",
                            "schema-name": "public",
                            "is-view": False,
                            "row-count": 0,
                        },
                        ("properties", "our_money_pk"): {
                            "inclusion": "automatic",
                            "sql-datatype": "money",
                            "selected-by-default": True,
                        },
                        ("properties", "our_money"): {
                            "inclusion": "available",
                            "sql-datatype": "money",
                            "selected-by-default": True,
                        },
...
class TestArraysTable(unittest.TestCase):
    ...
    def setUp(self):
        table_spec = {
            "columns": [
                {"name": "our_int_array_pk", "type": "integer[]", "primary_key": True},
...
class TestArraysLikeTable(unittest.TestCase):
    ...
    def test_catalog(self):
        ...
        chicken_streams = [
            s for s in streams if s["tap_stream_id"] == "public-LIKE CHICKEN TIMES"
        ]
        self.assertEqual(len(chicken_streams), 1)
        stream_dict = chicken_streams[0]
        stream_dict.get("metadata").sort(key=lambda md: md["breadcrumb"])
