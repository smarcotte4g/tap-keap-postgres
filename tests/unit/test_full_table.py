...
from tap_keap_postgres.sync_strategies.full_table import sync_view
...
class TestFullTable(TestCase):
    ...
    def test_sync_view(self):
        ...
        md_map = {
            (): {"schema-name": "pg_catalog", "replication-key": "oid"},
        ...
        expected_output_without_version = {
            "bookmarks": {
                "foo-bar": {"foo": "bar", "lsn": 4, "version": mocked_time_value * 1000}
            }
        }
        with patch("time.time") as mocked_time:
            mocked_time.return_value = mocked_time_value
            actual_output = sync_view(
                self.conn_config, stream, state, desired_columns, md_map
            )
