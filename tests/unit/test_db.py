...
from tap_keap_postgres import db
...
class TestDbFunctions(unittest.TestCase):
    ...
    def test_value_to_singer_value(self):
        ...
        self.assertEqual(
            db.selected_value_to_singer_value_impl('{"test": 123}', "json"),
            {"test": 123},
        )
        self.assertEqual(
            db.selected_value_to_singer_value_impl('{"test": 123}', "jsonb"),
            {"test": 123},
        )
        ...
        self.assertEqual(
            db.selected_value_to_singer_value_impl(
                "12:00:00-0800", "time with time zone"
            ),
            "20:00:00",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            db.selected_value_to_singer_value_impl(
                "2020-05-01T12:00:00-0800", "timestamp with time zone"
            ),
        ...
        self.assertEqual(
            db.selected_value_to_singer_value_impl(False, "boolean"), False
        )

        self.assertEqual(
            db.selected_value_to_singer_value_impl({"foo": "bar"}, "hstore"),
            {"foo": "bar"},
        )

        self.assertEqual(db.selected_value_to_singer_value_impl(3.14, "foo"), 3.14)
        self.assertEqual(
            db.selected_value_to_singer_value_impl(float("nan"), "foo"), None
        )
        self.assertEqual(
            db.selected_value_to_singer_value_impl(float("inf"), "foo"), None
        )

        self.assertEqual(
            db.selected_value_to_singer_value_impl(decimal.Decimal(7), "foo"),
            decimal.Decimal(7),
        )
        self.assertEqual(
            db.selected_value_to_singer_value_impl(decimal.Decimal("nan"), "foo"), None
        )

        self.assertEqual(
            db.selected_value_to_singer_value_impl(datetime.date(2022, 11, 11), "foo"),
            "2022-11-11T00:00:00+00:00",
        )
        self.assertEqual(
            db.selected_value_to_singer_value_impl(datetime.time(11, 11, 11), "foo"),
            "11:11:11",
        )
    ...
    def test_prepare_columns_for_select_sql_with_timestamp_ntz_column(self):
        self.assertEqual(
            ...
            db.prepare_columns_for_select_sql(
                "my_column",
                {
    ...
    def test_prepare_columns_for_select_sql_with_not_timestamp_column(self):
        self.assertEqual(
            ' "my_column" ',
            db.prepare_columns_for_select_sql(
                "my_column", {("properties", "my_column"): {"sql-datatype": "int"}}
            ),
        )
    ...
    def test_prepare_columns_for_select_sql_with_column_not_in_map(self):
        self.assertEqual(
            ' "my_column" ',
            db.prepare_columns_for_select_sql(
                "my_column", {("properties", "nope"): {"sql-datatype": "int"}}
            ),
        )
    ...
    def test_selected_value_to_singer_value_impl_with_non_empty_json_returns_equivalent_dict(
        ...
        output = db.selected_value_to_singer_value_impl(
            '{"key1": "A", "key2": [{"kk": "yo"}, {}]}', "json"
        )
    ...
    def test_numeric_precision_returns_default_value_if_column_numeric_precision_greater_than_max_precision(
        self,
    ):
        class Column:
    ...
    def test_numeric_scale_returns_default_value_if_column_numeric_scale_greater_than_max_scale(
        self,
    ):
        class Column:
