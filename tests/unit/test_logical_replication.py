...
from tap_keap_postgres.sync_strategies import logical_replication
from tap_keap_postgres.sync_strategies.logical_replication import (
...
class PostgresCurReplicationSlotMock:
    ...
    def execute(self, sql):
        ...
        if (
            sql
            == f"SELECT * FROM pg_replication_slots WHERE slot_name = '{self.existing_slot_name}'"
        ):
...
class TestLogicalReplication(unittest.TestCase):
    ...
    def test_streams_to_wal2json_tables(self):
        ...
        streams = [
            ...
            {
                "metadata": [
                    {"metadata": {"schema-name": "Case Sensitive Schema With Space"}}
                ],
                "table_name": "CaseSensitiveTable",
            },
            {
    ...
    def test_generate_replication_slot_name(self):
        ...
        self.assertEqual(
            logical_replication.generate_replication_slot_name("some_db", "some_tap"),
            "pipelinewise_some_db_some_tap",
        )

        # Provide database name, tap_id and prefix
        self.assertEqual(
            logical_replication.generate_replication_slot_name(
        ...
        self.assertEqual(
            logical_replication.generate_replication_slot_name("some-db", "some-tap"),
            "pipelinewise_some_db_some_tap",
        )
    ...
    def test_locate_replication_slot_by_cur(self):
        ...
        cursor = PostgresCurReplicationSlotMock(
            existing_slot_name="pipelinewise_some_db"
        )
        self.assertEqual(
            logical_replication.locate_replication_slot_by_cur(
                cursor, "some_db", "some_tap"
            ),
            "pipelinewise_some_db",
        )
        ...
        cursor = PostgresCurReplicationSlotMock(
            existing_slot_name="pipelinewise_some_db_some_tap"
        )
        self.assertEqual(
            logical_replication.locate_replication_slot_by_cur(
                cursor, "some_db", "some_tap"
        ...
        self.assertEqual(
            logical_replication.locate_replication_slot_by_cur(cursor, "some_db"),
            "pipelinewise_some_db",
        )
    ...
    def test_consume_with_payload_kind_is_not_supported_expect_exception(self):
        with self.assertRaises(UnsupportedPayloadKindError):
            logical_replication.consume_message(
                ...
                {},
                self.WalMessage(
                    payload='{"action":"truncate", "schema": "myschema", "table": "mytable"}',
                    data_start="some lsn",
                ),
                None,
                {},
    ...
    @patch("tap_keap_postgres.logical_replication.singer.write_message")
    @patch("tap_keap_postgres.logical_replication.sync_common.send_schema_message")
    @patch("tap_keap_postgres.logical_replication.refresh_streams_schema")
        ...
        streams = [
            {
                "tap_stream_id": "myschema-mytable",
                "stream": "mytable",
                "schema": {"properties": {"id": {}, "date_created": {}}},
                "metadata": [
                    {
                        "breadcrumb": [],
                        "metadata": {
    ...
    def test_selected_value_to_singer_value_impl_with_timestamp_ntz_value_as_string_expect_iso_format(
        self,
    ):
        output = logical_replication.selected_value_to_singer_value_impl(
            "2020-09-01 20:10:56", "timestamp without time zone", None
        )
    ...
    def test_selected_value_to_singer_value_impl_with_timestamp_ntz_value_as_datetime_expect_iso_format(
        ...
        output = logical_replication.selected_value_to_singer_value_impl(
            datetime(2020, 9, 1, 20, 10, 59), "timestamp without time zone", None
        )

        self.assertEqual("2020-09-01T20:10:59+00:00", output)

    def test_selected_value_to_singer_value_impl_with_timestamp_ntz_value_as_string_out_of_range_1(
        self,
    ):
        """
        ...
        """
        output = logical_replication.selected_value_to_singer_value_impl(
            "10000-09-01 20:10:56", "timestamp without time zone", None
        )
    ...
    def test_selected_value_to_singer_value_impl_with_timestamp_ntz_value_as_string_BC(
        self,
    ):
        """
        ...
        """
        output = logical_replication.selected_value_to_singer_value_impl(
            "1000-09-01 20:10:56 BC", "timestamp without time zone", None
        )

        self.assertEqual("9999-12-31T23:59:59.999+00:00", output)

    def test_selected_value_to_singer_value_impl_with_timestamp_ntz_value_as_string_AC(
        self,
    ):
        """
        ...
        """
        output = logical_replication.selected_value_to_singer_value_impl(
            "1000-09-01 20:10:56 AC", "timestamp without time zone", None
        )

        self.assertEqual("9999-12-31T23:59:59.999+00:00", output)

    def test_selected_value_to_singer_value_impl_with_timestamp_ntz_value_as_string_min(
        self,
    ):
        output = logical_replication.selected_value_to_singer_value_impl(
            "0001-01-01 00:00:00.000123", "timestamp without time zone", None
        )

        self.assertEqual("0001-01-01T00:00:00.000123+00:00", output)

    def test_selected_value_to_singer_value_impl_with_timestamp_ntz_value_as_string_max(
        self,
    ):
        output = logical_replication.selected_value_to_singer_value_impl(
            "9999-12-31 23:59:59.999999", "timestamp without time zone", None
        )
    ...
    def test_selected_value_to_singer_value_impl_with_timestamp_ntz_value_as_datetime_min(
        ...
        output = logical_replication.selected_value_to_singer_value_impl(
            datetime(1, 1, 1, 0, 0, 0, 123), "timestamp without time zone", None
        )
    ...
    def test_selected_value_to_singer_value_impl_with_timestamp_tz_value_as_string_out_of_range_1(
        self,
    ):
        """
        ...
        """
        output = logical_replication.selected_value_to_singer_value_impl(
            "10000-09-01 20:10:56+06", "timestamp with time zone", None
        )

        self.assertEqual("9999-12-31T23:59:59.999+00:00", output)

    def test_selected_value_to_singer_value_impl_with_timestamp_tz_value_as_string_out_of_range_2(
        self,
    ):
        """
        ...
        """
        output = logical_replication.selected_value_to_singer_value_impl(
            "0000-09-01 20:10:56+01", "timestamp with time zone", None
        )

        self.assertEqual("9999-12-31T23:59:59.999+00:00", output)

    def test_selected_value_to_singer_value_impl_with_timestamp_tz_value_as_string_BC(
        self,
    ):
        """
        ...
        """
        output = logical_replication.selected_value_to_singer_value_impl(
            "1000-09-01 20:10:56+05 BC", "timestamp with time zone", None
        )
    ...
    def test_selected_value_to_singer_value_impl_with_timestamp_tz_value_as_string_min(
        self,
    ):
        output = logical_replication.selected_value_to_singer_value_impl(
            "0001-01-01 00:00:00.000123+04", "timestamp with time zone", None
        )
    ...
    def test_selected_value_to_singer_value_impl_with_timestamp_tz_value_as_datetime_max(
        ...
        output = logical_replication.selected_value_to_singer_value_impl(
            datetime(9999, 12, 31, 23, 59, 59, 999999, tzinfo=tzoffset(None, -14400)),
            "timestamp with time zone",
            None,
        )
    ...
    def test_selected_value_to_singer_value_impl_with_date_value_as_string_expect_iso_format(
        self,
    ):
        output = logical_replication.selected_value_to_singer_value_impl(
            "2021-09-07", "date", None
        )

        self.assertEqual("2021-09-07T00:00:00+00:00", output)

    def test_selected_value_to_singer_value_impl_with_date_value_as_string_out_of_range(
        self,
    ):
        """
    ...
    def test_row_to_singer_message(self):
        ...
        md_map = {
            ...
            ("properties", "c_timestamp_ntz_1"): {
                "sql-datatype": "timestamp without time zone"
            },
            ("properties", "c_timestamp_ntz_2"): {
                "sql-datatype": "timestamp without time zone"
            },
            ("properties", "c_timestamp_ntz_3"): {
                "sql-datatype": "timestamp without time zone"
            },
            ("properties", "c_timestamp_ntz_4"): {
                "sql-datatype": "timestamp without time zone"
            },
            ("properties", "c_timestamp_tz_1"): {
                "sql-datatype": "timestamp with time zone"
            },
            ("properties", "c_timestamp_tz_2"): {
                "sql-datatype": "timestamp with time zone"
            },
            ("properties", "c_timestamp_tz_3"): {
                "sql-datatype": "timestamp with time zone"
            },
            ("properties", "c_timestamp_tz_4"): {
                "sql-datatype": "timestamp with time zone"
            },
        }

        output = logical_replication.row_to_singer_message(
            stream,
            row,
            1000,
            columns,
    ...
    def test_selected_value_to_singer_value_impl_with_empty_json_returns_empty_dict(
        self,
    ):
        output = logical_replication.selected_value_to_singer_value_impl(
            "{}", "json", None
    ...
    def test_selected_value_to_singer_value_impl_with_non_empty_json_returns_equivalent_dict(
        self,
    ):
        output = logical_replication.selected_value_to_singer_value_impl(
            '{"key1": "A", "key2": [{"kk": "yo"}, {}]}', "json", None
        )
    ...
    def test_fetch_current_lsn_raises_exception_on_different_versions_of_pg(
        self, mocked_connect
    ):
        ...
        for version in test_versions:
            ...
            self.assertRaises(
                Exception, logical_replication.fetch_current_lsn, self.conn_info
            )

    @patch("tap_keap_postgres.sync_strategies.logical_replication.get_pg_version")
    @patch("psycopg2.connect")
    def test_fetch_current_lsn(self, mocked_connect, mocked_pg_version):
        ...
        # Look at tap_keap_postgres.sync_strategies.logical_replication.lsn_to_int to find out why!
    ...
    def test_add_automatic_properties_if_debug_lsn_is_off(self):
        ...
        with self.assertLogs(level="DEBUG") as foo_log:
            actual_stream = logical_replication.add_automatic_properties(
                stream=stream, debug_lsn=False
            )
            self.assertDictEqual(expected_stream, actual_stream)
            self.assertEqual(
                ["DEBUG:tap_keap_postgres:debug_lsn is OFF"], foo_log.output
            )
    ...
    def test_add_automatic_properties_if_debug_lsn_is_on(self):
        ...
        with self.assertLogs(level="DEBUG") as foo_log:
            actual_stream = logical_replication.add_automatic_properties(
                stream=stream, debug_lsn=True
            )
            self.assertDictEqual(expected_stream, actual_stream)
            self.assertEqual(
                ["DEBUG:tap_keap_postgres:debug_lsn is ON"], foo_log.output
            )
    ...
    def test_get_stream_version_raises_exception_if_version_is_none(self):
        ...
        self.assertEqual(
            f"version not found for log miner {tap_stream_id}", str(exp.exception)
        )
    ...
    def test_create_array_elem(self, mocked_connect):
        ...
        for sql_datatype, elem, expected_output in test_values:
            ...
            actual_output = logical_replication.create_array_elem(
                elem, sql_datatype, self.conn_info
            )
            self.assertEqual(expected_output, actual_output)
    ...
    def test_selected_array_to_singer_value(self):
        ...
        test_values = [
            (
                ...
                ["2022-11-11T00:00:00+00:00", "2022-11-12T00:00:00+00:00"],
            ),
            ("2022-11-11", "2022-11-11T00:00:00+00:00"),
        ]
        for elem, expected_output in test_values:
            actual_output = logical_replication.selected_array_to_singer_value(
                elem, sql_datatype, self.conn_info
            )
            self.assertEqual(expected_output, actual_output)
    ...
    def test_selected_value_to_singer_value(self, mocked_connect):
        ...
        test_values = [("bar", "{foo}", "{foo}"), ("text[]", "{foo}", ["foo"])]
        for sql_datatype, elem, expected_output in test_values:
            actual_output = logical_replication.selected_value_to_singer_value(
                elem, sql_datatype, self.conn_info
            )
            self.assertEqual(expected_output, actual_output)
    ...
    def test_row_to_singer_message_raises_exception_if_no_sql_datatype_in_md_map(self):
        ...
        self.assertEqual(
            f"Unable to find sql-datatype for stream {stream}", str(exp.exception)
        )
    ...
    def test_row_to_singer_message(self):
        ...
        md_map = {
            ("properties", "foo"): {"sql-datatype": "[foo]", "schema-name": "bar"}
        }
        actual_output = logical_replication.row_to_singer_message(
            stream, row, version, columns, time_extracted, md_map, self.conn_info
        )

        expected_output = singer.RecordMessage(
            stream="None-1", record={"foo": "foo"}, version=version
        )
        self.assertEqual(expected_output, actual_output)
    ...
    def test_impl_if_sql_datatype_is_money(self):
        ...
        actual_output = logical_replication.selected_value_to_singer_value_impl(
            elem, og_sql_datatype, conn_info
        )

        self.assertEqual(expected_output, actual_output)

    def test_impl_if_timestamp_with_time_zone_as_datatype_and_greater_than_fallback_datetime(
        self,
    ):
    ...
    def test_impl_datatype_timestamp_with_time_zone_and_parsed_elm_greater_than_and_not_datetime(
        self,
    ):
        """Test selected_value_to_singer_value_impl if datatype is
    ...
    def test_impl_with_sql_datatype_is_time_with_time_zone_and_elem_starts_with_24(
        self,
    ):
    ...
    def test_impl_with_sql_datatype_is_time_without_time_zone_elem_starts_with_24(self):
        ...
        conn_info = None
        actual_output = logical_replication.selected_value_to_singer_value_impl(
            test_elem, og_sql_datatype, conn_info
        )
    ...
    def test_impl_with_sql_datatype_is_hstore(self, mocked_connect):
        ...
        actual_output = logical_replication.selected_value_to_singer_value_impl(
            hstore_elem, og_sql_datatype, self.conn_info
        )
    ...
    @patch(
        "tap_keap_postgres.sync_strategies.logical_replication.refresh_streams_schema"
    )
    @patch(
        "tap_keap_postgres.sync_strategies.logical_replication.sync_common.send_schema_message"
    )
    def test_consume_message_if_payload_kind_insert_or_update(self, *args):
        ...
        actual_output = logical_replication.consume_message(
            streams, state, insert_msg, time_extracted, self.conn_info
        )
        self.assertDictEqual(expected_output, actual_output)

        actual_output = logical_replication.consume_message(
            streams, state, update_msg, time_extracted, self.conn_info
        )
        self.assertDictEqual(expected_output, actual_output)
    ...
    def test_consume_message_raises_exception_if_delete_and_no_datatype_for_stream(
        self, *args
    ):
        """Test consume_message raises exception if kind is delete and could not a datatype for the stream"""

        delete_msg = self.WalMessage(
            data_start="foo_start",
            payload=json.dumps(
                {
                    "schema": "foo",
        ...
        with self.assertRaises(Exception) as exp:
            logical_replication.consume_message(
                streams, state, delete_msg, time_extracted, self.conn_info
            )

        self.assertEqual(expected_message, str(exp.exception))

    @patch(
        "tap_keap_postgres.sync_strategies.logical_replication.locate_replication_slot"
    )
    @patch("tap_keap_postgres.sync_strategies.logical_replication.get_pg_version")
    @patch("psycopg2.connect")
    def test_sync_tables_raises_exception_if_psycopg2_programming_error(
        self, mocked_connect, mocked_version, mocked_locate_rep_slot
    ):
        """Test if sync_tables raises exception on psycopg2.ProgrammingError"""

        mocked_start_replication = (
            mocked_connect.return_value.cursor.return_value.start_replication
        )
        ...
        expected_message = (
            "Unable to start replication with logical replication"
            f" (slot {test_replication_slot})"
        )
        mocked_version.return_value = 150000
        mocked_start_replication.side_effect = psycopg2.ProgrammingError(
        ...
        with self.assertRaises(Exception) as exp:
            logical_replication.sync_tables(
                self.conn_info, self.logical_streams, state, end_lsn, state_file
            )
        self.assertEqual(expected_message, str(exp.exception))

    @patch("tap_keap_postgres.sync_strategies.logical_replication.datetime.datetime")
    @patch(
        "tap_keap_postgres.sync_strategies.logical_replication.locate_replication_slot"
    )
    @patch("tap_keap_postgres.sync_strategies.logical_replication.get_pg_version")
    @patch("psycopg2.connect")
    def test_sync_tables_if_poll_duration_greater_than_logical_poll_total_seconds(
        self, mocked_connect, mocked_version, mocked_locate_rep_slot, mocked_datetime
    ):
        ...
        mocked_datetime.utcnow().__sub__().total_seconds.return_value = (
            test_poll_duration
        )
        mocked_start_replication = (
            mocked_connect.return_value.cursor.return_value.start_replication
        ...
        actual_output = logical_replication.sync_tables(
            self.conn_info, self.logical_streams, state, end_lsn, state_file
        )
    ...
    @patch("tap_keap_postgres.sync_strategies.logical_replication.get_pg_version")
    @patch("psycopg2.connect")
    def test_sync_tables_if_reached_max_run_seconds(
        self, mocked_connect, mocked_version, mocked_locate_rep_slot, mocked_datetime
    ):
    ...
    @patch("tap_keap_postgres.sync_strategies.logical_replication.get_pg_version")
    @patch("psycopg2.connect")
    def test_sync_tables_raise_exception_if_error_in_message_read(
        self, mocked_connect, mocked_version, _
    ):
        ...
        mocked_connect.return_value.cursor.return_value.read_message.side_effect = (
    ...
    @patch("tap_keap_postgres.sync_strategies.logical_replication.get_pg_version")
    @patch("psycopg2.connect")
    def test_sync_tables_if_break_at_end_lsn_and_msg_data_start_greater_than_end_lsn(
        self, mocked_connect, mocked_version, mocked_locate_rep_slot
    ):
        ...
        mocked_connect.return_value.cursor.return_value.read_message.return_value = (
            test_message()
        )

        mocked_locate_rep_slot.return_value = "mocked_value_for_replication_slot"
        expected_log_message = (
            "INFO:tap_keap_postgres:Breaking - latest wal message "
    ...
    @patch("tap_keap_postgres.sync_strategies.logical_replication.get_pg_version")
    @patch("tap_keap_postgres.sync_strategies.logical_replication.select")
    @patch("psycopg2.connect")
    def test_sync_tables_if_no_message_and_raised_interrupted_error(
        self, mocked_connect, mocked_select, mocked_version, mocked_locate_rep_slot
    ):
    ...
    @patch("tap_keap_postgres.sync_strategies.logical_replication.get_pg_version")
    @patch("psycopg2.connect")
    def test_sync_tables_if_msg_and_some_specific_cases_for_lsn(
        self, mocked_connect, mocked_version, mocked_locate_rep_slot
    ):
        ...
        state = {
            "bookmarks": {
                "foo-bar": {"foo": "bar", "version": "foo", "lsn": lsn_committed}
            }
        }
        ...
        self.assertEqual(state, actual_output)
        mocked_send_feedback = (
            mocked_connect.return_value.cursor.return_value.send_feedback
        )
        mocked_send_feedback.assert_called_with(
            write_lsn=test_message.data_start,
