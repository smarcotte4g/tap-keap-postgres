...
def open_connection(conn_config, logical_replication=False, prioritize_primary=False):
    ...
    if (
        conn_config["use_secondary"]
        and not prioritize_primary
        and not logical_replication
    ):
        # Try to use replica but fallback to primary if keys are missing. This is the same behavior as
        # https://github.com/transferwise/pipelinewise/blob/master/pipelinewise/fastsync/commons/tap_keap_postgres.py#L129
        cfg.update(
            {
