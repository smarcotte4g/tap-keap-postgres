...
import tap_keap_postgres.sync_strategies.common as sync_common
from singer import get_bookmark, metadata, utils
from singer.catalog import Catalog
from tap_keap_postgres.discovery_utils import discover_db
from tap_keap_postgres.stream_utils import (
    any_logical_streams,
    clear_state_on_replication_change,
    dump_catalog,
    is_selected_via_metadata,
    refresh_streams_schema,
)
from tap_keap_postgres.sync_strategies import (
    full_table,
    incremental,
    logical_replication,
)
...
def sync_method_for_streams(streams, state, default_replication_method):
    ...
    for stream in streams:
        ...
        desired_columns = [
            c
            for c in stream["schema"]["properties"].keys()
        ...
        if replication_method == "LOG_BASED" and stream_metadata.get((), {}).get(
            "is-view"
        ):
            raise Exception(
                f"Logical Replication is NOT supported for views. "
        ...
        elif get_bookmark(state, stream["tap_stream_id"], "xmin") and get_bookmark(
        ...
        # inconsistent state
        elif get_bookmark(state, stream["tap_stream_id"], "xmin") and not get_bookmark(
            state, stream["tap_stream_id"], "lsn"
        ):
            raise Exception(
        ...
        elif not get_bookmark(
            state, stream["tap_stream_id"], "xmin"
        ) and not get_bookmark(state, stream["tap_stream_id"], "lsn"):
...
def sync_traditional_stream(conn_config, stream, state, sync_method, end_lsn):
    ...
    """
    LOGGER.info(
        "Beginning sync of stream(%s) with sync method(%s)",
        stream["tap_stream_id"],
        sync_method,
    )
    md_map = metadata.to_map(stream["metadata"])
...
def sync_logical_streams(conn_config, logical_streams, state, end_lsn, state_file):
    ...
    if logical_streams:
        LOGGER.info(
            "Pure Logical Replication upto lsn %s for (%s)",
            end_lsn,
        ...
        state = logical_replication.sync_tables(
            conn_config, logical_streams, state, end_lsn, state_file
        )
...
def do_sync(conn_config, catalog, default_replication_method, state, state_file=None):
    ...
    if currently_syncing:
        ...
        currently_syncing_stream = list(
            filter(
                lambda s: s["tap_stream_id"] == currently_syncing, traditional_streams
            )
        )
    ...
    logical_streams.sort(
        key=lambda s: metadata.to_map(s["metadata"]).get(()).get("database-name")
    )
    for dbname, streams in itertools.groupby(
        ...
        state = sync_logical_streams(
            conn_config, list(streams), state, end_lsn, state_file
        )
    return state
...
def main_impl():
    ...
    conn_config = {
        ...
        "logical_poll_total_seconds": float(
            args.config.get("logical_poll_total_seconds", 0)
        ),
    ...
    post_db.CURSOR_ITER_SIZE = int(
        args.config.get("itersize", post_db.CURSOR_ITER_SIZE)
    )
    ...
    if args.discover:
        ...
        do_sync(
            conn_config,
            args.catalog.to_dict() if args.catalog else args.properties,
