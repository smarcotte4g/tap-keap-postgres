...
def sync_table(conn_info, stream, state, desired_columns, md_map):
    ...
    escaped_columns = map(
        partial(post_db.prepare_columns_for_select_sql, md_map=md_map), desired_columns
    )
    ...
    with metrics.record_counter(None) as counter:
        with post_db.open_connection(conn_info) as conn:
            ...
            with conn.cursor(
                cursor_factory=psycopg2.extras.DictCursor, name="pipelinewise"
            ) as cur:
...
def _get_select_sql(params):
    ...
    where_statement = (
        f"WHERE {replication_key} >= '{replication_key_value}'::{replication_key_sql_datatype}"
