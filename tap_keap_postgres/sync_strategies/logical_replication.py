...
from tap_keap_postgres.stream_utils import refresh_streams_schema
...
def add_automatic_properties(stream, debug_lsn: bool = False):
    stream["schema"]["properties"]["_sdc_deleted_at"] = {
        "type": ["null", "string"],
        "format": "date-time",
    }
...
def selected_value_to_singer_value_impl(elem, og_sql_datatype, conn_info):
    ...
    if sql_datatype == "timestamp with time zone":
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                # compare the date in UTC tz to the max allowed
                if parsed.astimezone(pytz.UTC).replace(tzinfo=None) > datetime.datetime(
    ...
    if sql_datatype == "date":
        ...
        try:
            ...
            if match and int(match.group(1)) > 9999:
                LOGGER.warning(
                    "datetimes cannot handle years past 9999, returning %s for %s",
                    FALLBACK_DATE,
                    elem,
                )
    ...
    if sql_datatype == "time with time zone":
        ...
        if elem_obj.utcoffset() != datetime.timedelta(seconds=0):
            LOGGER.warning(
                "time with time zone values are converted to UTC: %s", og_sql_datatype
            )
...
def consume_message(streams, state, msg, time_extracted, conn_info):
    ...
    diff = set()
    if action in {"I", "U"}:
        diff = {column["name"] for column in payload["columns"]}.difference(
    ...
    if diff:
        LOGGER.info(
            'Detected new columns "%s", refreshing schema of stream %s',
            diff,
...
def locate_replication_slot_by_cur(cursor, dbname, tap_id=None):
    ...
    cursor.execute(
        f"SELECT * FROM pg_replication_slots WHERE slot_name = '{slot_name_v16}'"
    )
    if len(cursor.fetchall()) == 1:
    ...
    raise ReplicationSlotNotFoundError(
        f"Unable to find replication slot {slot_name_v16}"
    )
    ...
    with post_db.open_connection(conn_info, False, True) as conn:
        with conn.cursor() as cur:
            return locate_replication_slot_by_cur(
                cur, conn_info["dbname"], conn_info["tap_id"]
            )
...
def sync_tables(conn_info, logical_streams, state, end_lsn, state_file):
    ...
    logical_poll_total_seconds = (
        conn_info["logical_poll_total_seconds"] or 10800
    )  # 3 hours
    poll_interval = 10
    ...
    if version >= 120000:
        ...
        LOGGER.info(
            "Set session wal_sender_timeout = %i milliseconds", wal_sender_timeout
        )
        cur.execute(f"SET SESSION wal_sender_timeout = {wal_sender_timeout}")

    try:
        LOGGER.info(
            "Request wal streaming from %s to %s (slot %s)",
    ...
    try:
        while True:
            ...
            if poll_duration > logical_poll_total_seconds:
                LOGGER.info(
                    "Breaking - %i seconds of polling with no data", poll_duration
                )
                break

            if datetime.datetime.utcnow() >= (
                start_run_timestamp + datetime.timedelta(seconds=max_run_seconds)
            ):
            ...
            if msg:
                ...
                state = consume_message(
                    logical_streams, state, msg, time_extracted, conn_info
                )
                ...
                if lsn_currently_processing is None:
                    ...
                    LOGGER.info(
                        "First wal message received is %s",
                    ...
                    if lsn_processed_count >= UPDATE_BOOKMARK_PERIOD:
                        ...
                        for s in logical_streams:
                            state = singer.write_bookmark(
                                state, s["tap_stream_id"], "lsn", lsn_last_processed
                            )
            ...
            if datetime.datetime.utcnow() >= (
                ...
                if lsn_currently_processing is None:
                    ...
                    LOGGER.info(
                        "Lastest wal message received was %s",
                        int_to_lsn(lsn_last_processed),
                    )
                    try:
