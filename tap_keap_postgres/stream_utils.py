...
def refresh_streams_schema(conn_config: Dict, streams: List[Dict]):
    ...
    with open_connection(conn_config) as conn:
        ...
        for idx, stream in enumerate(streams):
            # update schema
            streams[idx]["schema"] = copy.deepcopy(
                new_discovery[stream["tap_stream_id"]]["schema"]
            )
            ...
            for idx_met, metadatum in enumerate(
                new_discovery[stream["tap_stream_id"]]["metadata"]
            ):
                if not metadatum["breadcrumb"]:
                    meta.update(
                        new_discovery[stream["tap_stream_id"]]["metadata"][idx_met][
                            "metadata"
                        ]
                    )
                    del meta["table-key-properties"]
                    new_discovery[stream["tap_stream_id"]]["metadata"][idx_met][
                        "metadata"
                    ] = meta

            # 2nd step: now copy all the metadata from the updated new discovery to the original stream
            streams[idx]["metadata"] = copy.deepcopy(
                new_discovery[stream["tap_stream_id"]]["metadata"]
            )
