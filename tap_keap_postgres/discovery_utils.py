...
def discover_columns(connection, table_info):
    ...
    for schema_name in table_info.keys():
        for table_name in table_info[schema_name].keys():
            ...
            metadata.write(
                mdata, (), "row-count", table_info[schema_name][table_name]["row_count"]
            )
            metadata.write(
                mdata, (), "is-view", table_info[schema_name][table_name].get("is_view")
            )
            ...
            schema = {"type": "object", "properties": column_schemas, "definitions": {}}
            ...
            for c_name in column_schemas.keys():
                ...
                if column_schemas[c_name].get("type") is None:
                    mdata = metadata.write(
                        mdata, ("properties", c_name), "inclusion", "unsupported"
                    )
                    mdata = metadata.write(
                        mdata, ("properties", c_name), "selected-by-default", False
                    )
                elif table_info[schema_name][table_name]["columns"][
                    c_name
                ].is_primary_key:
                    mdata = metadata.write(
                        mdata, ("properties", c_name), "inclusion", "automatic"
                    )
                    mdata = metadata.write(
                        mdata, ("properties", c_name), "selected-by-default", True
                    )
                else:
                    mdata = metadata.write(
                        mdata, ("properties", c_name), "inclusion", "available"
                    )
                    mdata = metadata.write(
                        mdata, ("properties", c_name), "selected-by-default", True
                    )
...
def include_array_schemas(columns, schema):
    ...
    decimal_array_columns = [
        key for key, value in columns.items() if value.sql_data_type == "numeric[]"
    ]
...
def write_sql_data_type_md(mdata, col_info):
    ...
    if col_info.sql_data_type == "bit" and col_info.character_maximum_length > 1:
        ...
        mdata = metadata.write(
            mdata, ("properties", c_name), "sql-datatype", col_info.sql_data_type
        )
